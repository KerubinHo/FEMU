...
static void nvme_post_cqes_io(void *opaque)
{
    ...
    ntt = (ntt == 0) ? qemu_clock_get_ns(QEMU_CLOCK_REALTIME) + 5000 : ntt;
...
static uint16_t nvme_create_sq(NvmeCtrl *n, NvmeCmd *cmd)
{
    ...
    if (n->db_addr && n->eventidx_addr) {
        sq->db_addr = n->db_addr + 2 * sqid * (1 << (2  + n->db_stride));
        sq->eventidx_addr = n->eventidx_addr + 2 * sqid *
            (1 << (2 + n->db_stride));
        printf("Coperd,%s,%d,DBBUF,sq[%d]:db_addr=%" PRIu64 ",eventidx_addr=%" PRIu64 "\n", __func__, __LINE__, sqid, sq->db_addr, sq->eventidx_addr);
    }
...
static uint16_t nvme_create_cq(NvmeCtrl *n, NvmeCmd *cmd)
{
    ...
    if (n->db_addr && n->eventidx_addr) {
        ...
        cq->db_addr = n->db_addr + (2 * cqid + 1) * (1 << (2 + n->db_stride));
        cq->eventidx_addr = n->eventidx_addr + (2 * cqid + 1) *
            (1 << (2 + n->db_stride));
        printf("Coperd,%s,%d,DBBUF,cq[%d]:db_addr=%" PRIu64 ",eventidx_addr=%" PRIu64 "\n", __func__, __LINE__, cqid, cq->db_addr, cq->eventidx_addr);
    }
...
static uint16_t nvme_admin_cmd(NvmeCtrl *n, NvmeCmd *cmd, NvmeRequest *req)
{
    ...
    case NVME_ADM_CMD_DELETE_SQ:
        printf("Coperd, nvme_cmd_del_sq\n");
        return nvme_del_sq(n, cmd);
    case NVME_ADM_CMD_CREATE_SQ:
        printf("Coperd, nvme_cmd_create_sq\n");
        return nvme_create_sq(n, cmd);
    case NVME_ADM_CMD_DELETE_CQ:
        printf("Coperd, nvme_cmd_delete_cq\n");
        return nvme_del_cq(n, cmd);
    case NVME_ADM_CMD_CREATE_CQ:
        printf("Coperd, nvme_cmd_create_cq\n");
        return nvme_create_cq(n, cmd);
    case NVME_ADM_CMD_IDENTIFY:
        printf("Coperd, nvme_cmd_identify\n");
        return nvme_identify(n, cmd);
    case NVME_ADM_CMD_SET_FEATURES:
        printf("Coperd, nvme_cmd_set_features\n");
        return nvme_set_feature(n, cmd, req);
    case NVME_ADM_CMD_GET_FEATURES:
        printf("Coperd, nvme_cmd_get_features\n");
        return nvme_get_feature(n, cmd, req);
    case NVME_ADM_CMD_GET_LOG_PAGE:
        printf("Coperd, nvme_get_log_page\n");
        return nvme_get_log(n, cmd);
    case NVME_ADM_CMD_ASYNC_EV_REQ:
        printf("Coperd, nvme_cmd_async_ev_req\n");
        return nvme_async_req(n, cmd, req);
    case NVME_ADM_CMD_ABORT:
        printf("Coperd, nvme_cmd_abort\n");
        return nvme_abort_req(n, cmd, &req->cqe.n.result);
    case NVME_ADM_CMD_FORMAT_NVM:
        printf("Coperd, nvme_format_nvm\n");
        if (NVME_OACS_FORMAT & n->oacs) {
            return nvme_format(n, cmd);
        }
        return NVME_INVALID_OPCODE | NVME_DNR;
    case NVME_ADM_CMD_SET_DB_MEMORY:
        printf("Coperd, nvme_adm_cmd_set_db_memory\n");
        return nvme_set_db_memory(n, cmd);
    case FEMU_OC_ADM_CMD_IDENTITY:
        printf("Coperd, nvme_OC_cmd_identity\n");
            return femu_oc_identity(n, cmd);
    case FEMU_OC_ADM_CMD_GET_L2P_TBL:
        printf("Coperd, nvme_OC_get_l2p_tbl\n");
...
static void nvme_process_sq_io(void *opaque)
{
     ...
     */
    timer_mod(sq->timer, qemu_clock_get_ns(QEMU_CLOCK_REALTIME) + 5000);
