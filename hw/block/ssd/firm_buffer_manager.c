...
void INIT_IO_BUFFER(struct ssdstate *ssd)
{
	...
	write_buffer = (void*)calloc(ssd->ssdparams.WRITE_BUFFER_FRAME_NB, ssd->ssdparams.SECTOR_SIZE);
	write_buffer_end = write_buffer + ssd->ssdparams.WRITE_BUFFER_FRAME_NB*ssd->ssdparams.SECTOR_SIZE;	

	/* Allocation Read Buffer in DRAM */
	read_buffer = (void*)calloc(ssd->ssdparams.READ_BUFFER_FRAME_NB, ssd->ssdparams.SECTOR_SIZE);
	read_buffer_end = read_buffer + ssd->ssdparams.READ_BUFFER_FRAME_NB*ssd->ssdparams.SECTOR_SIZE;	
	...
	empty_write_buffer_frame = ssd->ssdparams.WRITE_BUFFER_FRAME_NB;
	empty_read_buffer_frame = ssd->ssdparams.READ_BUFFER_FRAME_NB; 
...
void TERM_IO_BUFFER(struct ssdstate *ssd)
{
	...
	FLUSH_EVENT_QUEUE_UNTIL(ssd, e_queue->tail);
...
{
	int i;
	wb_valid_array = (char*)calloc(ssd->ssdparams.WRITE_BUFFER_FRAME_NB , sizeof(char));
	if(wb_valid_array == NULL){
		printf("[%s] Calloc write buffer valid array fail. \n",__FUNCTION__);
		return;
	}

	for(i=0;i<ssd->ssdparams.WRITE_BUFFER_FRAME_NB;i++){
		wb_valid_array[i] = '0';
	}
}
...
void *SSD_THREAD_MAIN_LOOP(struct ssdstate *ssd, void *arg)
{
	while(1){
		...
		if(r_queue_full == 1){
			SECURE_READ_BUFFER(ssd);
			r_queue_full = 0;
		}
		else if(w_queue_full == 1){
			SECURE_WRITE_BUFFER(ssd);
			w_queue_full = 0;
		}
		else{
...
/* Check event queue depth */
	...
	if(io_type == READ){
		ret = ENQUEUE_READ(ssd, sector_nb, length);
	}
	else if(io_type == WRITE){
		ENQUEUE_WRITE(ssd, sector_nb, length);
	}
	else{
	...
	if(valid == VALID){	
		...
		if(io_type == READ){
			if(buf != NULL){
				...
				ret = FTL_READ(ssd, sector_nb, length);
			}
		}
		else if(io_type == WRITE){
			ret = FTL_WRITE(ssd, sector_nb, length);
		}
		else{
...
void DEQUEUE_COMPLETED_READ(struct ssdstate *ssd)
{
	...
	while(c_e_q_entry != NULL){
		...
		READ_DATA_FROM_BUFFER_TO_HOST(ssd, c_e_q_entry);
...
int64_t ENQUEUE_READ(struct ssdstate *ssd, int64_t sector_nb, unsigned int length)
{
	...
	if(e_queue->entry_nb == 0){
		...
		if(ret_e_q_entry != NULL){
			...
			ret = FLUSH_EVENT_QUEUE_UNTIL(ssd, ret_e_q_entry);
			...
			if(temp_e_q_entry->sector_nb <= sector_nb && \
				...
				COPY_DATA_TO_READ_BUFFER(ssd, new_e_q_entry, temp_e_q_entry);
			}
...
void ENQUEUE_WRITE(struct ssdstate *ssd, int64_t sector_nb, unsigned int length)
{
	...
	WRITE_DATA_TO_BUFFER(ssd, length);
	...
	while(e_q_entry != NULL){
		...
		if(e_q_entry->valid == VALID && CHECK_OVERWRITE(e_q_entry, sector_nb, length)==SUCCESS){
			...
			UPDATE_WB_VALID_ARRAY(ssd, e_q_entry, 'I');
		}
	...
	if(CHECK_SEQUENTIALITY(e_queue->tail, sector_nb)==SUCCESS){
		...
		UPDATE_WB_VALID_ARRAY_PARTIAL(ssd, e_queue->tail, 'I', invalid_len, 1);
...
void WRITE_DATA_TO_BUFFER(struct ssdstate *ssd, unsigned int length)
{
	...
	INCREASE_WB_SATA_POINTER(ssd, length);
...
void READ_DATA_FROM_BUFFER_TO_HOST(struct ssdstate *ssd, event_queue_entry* c_e_q_entry)
{
	...
	INCREASE_RB_SATA_POINTER(ssd, c_e_q_entry->length);
...
void COPY_DATA_TO_READ_BUFFER(struct ssdstate *ssd, event_queue_entry* dst_entry, event_queue_entry* src_entry)
{
	...
	while(count != offset){
		...
		if(GET_WB_VALID_ARRAY_ENTRY(ssd, src_buf)!='I'){
			count++;
		}

		src_buf = src_buf + ssd->ssdparams.SECTOR_SIZE;
	...
	while(count != dst_length){
		...
		memcpy(dst_buf, src_buf, ssd->ssdparams.SECTOR_SIZE);

		/* Increase offset */
		dst_buf = dst_buf + ssd->ssdparams.SECTOR_SIZE;
		src_buf = src_buf + ssd->ssdparams.SECTOR_SIZE;

		ftl_read_ptr = ftl_read_ptr + ssd->ssdparams.SECTOR_SIZE;
...
{
	return FLUSH_EVENT_QUEUE_UNTIL(ssd, e_queue->tail);
}
...
int64_t SECURE_READ_BUFFER(struct ssdstate *ssd)
{
	...
	if(last_read_entry != NULL){
		ret = FLUSH_EVENT_QUEUE_UNTIL(ssd, last_read_entry);
		DEQUEUE_COMPLETED_READ(ssd);
	}
	return ret;
}
...
char GET_WB_VALID_ARRAY_ENTRY(struct ssdstate *ssd, void* buffer_pointer)
{
	...
	int index = (int)(buffer_pointer - write_buffer)/ssd->ssdparams.SECTOR_SIZE;
...
void UPDATE_WB_VALID_ARRAY(struct ssdstate *ssd, event_queue_entry* e_q_entry, char new_value)
{
	...
	int index = (int)(p_buf - write_buffer)/ssd->ssdparams.SECTOR_SIZE;
	int count = 0;
	int length = e_q_entry->length;

	while(count != length){
		if(GET_WB_VALID_ARRAY_ENTRY(ssd, p_buf)=='V'){
			wb_valid_array[index] = new_value;	
			count++;
		}

		/* Increase index and buffer pointer */
		p_buf = p_buf + ssd->ssdparams.SECTOR_SIZE;
		index++;
		if(index == ssd->ssdparams.WRITE_BUFFER_FRAME_NB){
			p_buf = write_buffer;
			index = 0;
		} 
	}
...
void UPDATE_WB_VALID_ARRAY_PARTIAL(struct ssdstate *ssd, event_queue_entry* e_q_entry, char new_value, int length, int mode)
{
	...
	if(mode == 1){
		...
		while(count != offset){
			if(GET_WB_VALID_ARRAY_ENTRY(ssd, p_buf)!='I'){
				count++;
			}
			p_buf = p_buf + ssd->ssdparams.SECTOR_SIZE;
	...
	while(count != length){
		if(GET_WB_VALID_ARRAY_ENTRY(ssd, p_buf)!='I'){
			UPDATE_WB_VALID_ARRAY_ENTRY(ssd, p_buf, new_value);
			count++;
		}
...
void INCREASE_WB_SATA_POINTER(struct ssdstate *ssd, int entry_nb)
{
	int i;
#ifdef FIRM_IO_BUF_DEBUG
	int index = (int)(sata_write_ptr - write_buffer)/ssd->ssdparams.SECTOR_SIZE;
	...
	for(i=0; i<entry_nb; i++){
		...
		/* Update write buffer valid array */
		UPDATE_WB_VALID_ARRAY_ENTRY(ssd, sata_write_ptr, 'V');

		/* Increase sata write pointer */
		sata_write_ptr = sata_write_ptr + ssd->ssdparams.SECTOR_SIZE;
...
#ifdef FIRM_IO_BUF_DEBUG
	index = (int)(sata_write_ptr - write_buffer)/ssd->ssdparams.SECTOR_SIZE;
	printf("%d End.\n",index);
#endif
}
...
{
#ifdef FIRM_IO_BUF_DEBUG
	int index = (int)(sata_read_ptr - read_buffer)/ssd->ssdparams.SECTOR_SIZE;
	printf("[%s] Start: %d -> ",__FUNCTION__, index);
#endif
	int i;
	...
	for(i=0; i<entry_nb; i++){
		...
		sata_read_ptr = sata_read_ptr + ssd->ssdparams.SECTOR_SIZE;
...
#ifdef FIRM_IO_BUF_DEBUG
	index = (int)(sata_read_ptr - read_buffer)/ssd->ssdparams.SECTOR_SIZE;
	printf("%d End.\n",index);
#endif
}
...
{
#ifdef FIRM_IO_BUF_DEBUG
	int index = (int)(ftl_write_ptr - write_buffer)/ssd->ssdparams.SECTOR_SIZE;
	printf("[%s] Start: %d -> ",__FUNCTION__, index);
#endif
	int count = 0;
	char validity;
	...
	while(count != entry_nb){
		...
		validity = GET_WB_VALID_ARRAY_ENTRY(ssd, ftl_write_ptr);

		if(validity == 'V'){
			/* Update write buffer valid array */
			UPDATE_WB_VALID_ARRAY_ENTRY(ssd, ftl_write_ptr, 'F');

			count++;
		}

		/* Increase ftl pointer by SECTOR_SIZE */
		ftl_write_ptr = ftl_write_ptr + ssd->ssdparams.SECTOR_SIZE;
		if(ftl_write_ptr == write_buffer_end){
			ftl_write_ptr = write_buffer;
		}
	}
#ifdef FIRM_IO_BUF_DEBUG
	index = (int)(ftl_write_ptr - write_buffer)/ssd->ssdparams.SECTOR_SIZE;
	printf("%d End.\n",index);
#endif
}
...
{
#ifdef FIRM_IO_BUF_DEBUG
	int index = (int)(ftl_read_ptr - read_buffer)/ssd->ssdparams.SECTOR_SIZE;
	printf("[%s] Start: %d -> ",__FUNCTION__, index);
#endif
	int i;
...
#ifdef FIRM_IO_BUF_DEBUG
	index = (int)(ftl_read_ptr - read_buffer)/ssd->ssdparams.SECTOR_SIZE;
	printf("%d End.\n",index);
#endif
}
...
{
#ifdef FIRM_IO_BUF_DEBUG
	int index = (int)(write_limit_ptr - write_buffer)/ssd->ssdparams.SECTOR_SIZE;
	printf("[%s] Start: %d -> ",__FUNCTION__, index);
#endif
	/* Increase write limit pointer until ftl write pointer */
	do{
		/* Update write buffer valid array */
		UPDATE_WB_VALID_ARRAY_ENTRY(ssd, write_limit_ptr, '0');

		/* Incrase write limit pointer by SECTOR_SIZE */
		write_limit_ptr = write_limit_ptr + ssd->ssdparams.SECTOR_SIZE;
...
#ifdef FIRM_IO_BUF_DEBUG
	index = (int)(write_limit_ptr - write_buffer)/ssd->ssdparams.SECTOR_SIZE;
	printf("%d. End.\n",index);
#endif
}
...
{
#ifdef FIRM_IO_BUF_DEBUG
	int index = (int)(read_limit_ptr - read_buffer)/ssd->ssdparams.SECTOR_SIZE;
	printf("[%s] Start: %d -> ",__FUNCTION__, index);
#endif
	...
	do{
		...
		read_limit_ptr = read_limit_ptr + ssd->ssdparams.SECTOR_SIZE;
		if(read_limit_ptr == read_buffer_end){
			read_limit_ptr = read_buffer;
		}
	}while(read_limit_ptr != ftl_read_ptr);
#ifdef FIRM_IO_BUF_DEBUG
	index = (int)(read_limit_ptr - read_buffer)/ssd->ssdparams.SECTOR_SIZE;
	printf("%d End.\n",index);
#endif
}
